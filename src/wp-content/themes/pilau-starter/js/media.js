...
/**
 * Media API Javascript - mostly for custom media upload dialogs
 */

jQuery( document ).ready( function( $ ) {

	// Prepare the variable that holds our custom media managers.
	var pilau_media_frames = [];

	$( document.body ).on( 'click', '.pilau-upload-media-button', function( e ) {
		...
		if ( typeof pilau_upload_media[ 'dialog_title__' + field_id ] != 'undefined' ) {
			dialog_title = pilau_upload_media[ 'dialog_title__' + field_id ];
		}
		if ( typeof pilau_upload_media[ 'restrict_to_type__' + field_id ] != 'undefined' ) {
			restrict_to_type = pilau_upload_media[ 'restrict_to_type__' + field_id ];
		}

		// If the frame already exists, re-open it.
		if ( pilau_media_frames[ field_id ] ) {
			// Make sure the post attachment parameter is set correctly
			pilau_media_frames[ field_id ].uploader.uploader.param( 'post_id', 0 );
			// Open frame
			pilau_media_frames[ field_id ].open();
			return;
		} else {
			// Make sure the post attachment parameter is set correctly
		 ...
		 */
		pilau_media_frames[ field_id ] = wp.media.frames.slt_cf_media_frame = wp.media({
			/**
			 ...
			 */
			className: 'media-frame pilau-media-frame',
			 ...
			 * the script with the object 'pilau_upload_media' that holds our
			 ...
			 * own events here, but the default event will work just fine.
			 button: {
				text:  pilau_upload_media.button_text
			}
			 */
		 ...
		 */
		pilau_media_frames[ field_id ].on( 'select', function() {
			var preview_div, preview_size;

			// Grab our attachment selection and construct a JSON representation of the model.
			var media_attachment = pilau_media_frames[ field_id ].state().get( 'selection' ).first().toJSON();
			...
			// Load preview image
			preview_div.html( '' ).load( pilau_admin.ajaxurl, {
				id: 	media_attachment.id,
				size:	preview_size,
				action:	'pilau_upload_media_get_file'
			});
